/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   door_utils.c                                       :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: yettabaa <yettabaa@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/06/16 04:24:45 by yettabaa          #+#    #+#             */
/*   Updated: 2023/06/18 00:22:23 by yettabaa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include"cub3D.h"

void get_text_door(t_data *v)
{
    int i;
	int endian;
    char *itoa;
    char *path;
    void *tx_img;
    
    i = 0;
    v->door.DOOR_buff = malloc(sizeof(unsigned int*) * 34);
    if (!v->door.DOOR_buff)
        ft_error("Allocate door buffer failed");
    while (i <= 32)
    {
        itoa = ft_itoa(i+1);
        path = ft_strjoin3("./textures/DOOR/", itoa, ".xpm");
        // printf("%s\n",path);
        tx_img =  mlx_xpm_file_to_image(v->mlx.mlx, path, &v->door.DOOR_width, &v->door.DOOR_height);
        free(itoa);
        free(path);
        if(!tx_img)
            ft_error("Door PATH");
        v->door.DOOR_buff[i++] = (unsigned int *)mlx_get_data_addr(tx_img, &endian, &v->door.DOOR_line, &endian);
    }

}

void fill_door(t_data *v, int ind)
{    
    v->txt.buff = v->door.DOOR_buff[ind];
    v->txt.width = v->door.DOOR_width;
    v->txt.height = v->door.DOOR_height;
    v->txt.line = v->door.DOOR_line;   
}

// t_list_door *newdoor(t_data *v, double ryd, int flag)
// {
//     t_list_door *nd;
//     (void)v;
    
//     nd = malloc(sizeof(t_list_door));
//     if (!nd)
//         ft_error("Allocate new door failed");
// 	nd->type = DOOR;
// 	(flag == WALL) && (nd->type = WALL);
// 	nd->rydis = ryd;
//     nd->rydis_fbw = ryd * cos(Rad(v->orientation - v->ryc.ang));
// 	(flag == HORI || flag == VER) && (nd->hitw = flag);
// 	nd->x = v->x + ryd * cos(Rad(v->ryc.ang));
// 	nd->y = v->y + ryd * sin(Rad(v->ryc.ang));
// 	nd->next = NULL;
//     return(nd);
// }

// void	addoor(t_list_door **lst, t_list_door *new)
// {
// 	if (!lst)
// 		return ;
// 	if (*lst == NULL)
// 	{
// 		*lst = new;
// 		return ;
// 	}
// 	new->next = *lst;
// 	*lst = new;
// }

// void	clear_door(t_list_door **lst)
// {
// 	t_list_door	*c;

// 	if (!lst)
// 		return ;
// 	c = *lst;
// 	while (c)
// 	{
// 		if (c->next == NULL)
// 		{
// 			free((void*)*lst);
// 			break ;
// 		}
// 		c = c->next;
// 		free((void*)*lst);
// 		*lst = c;
// 	}
// 	*lst = NULL;
// }
t_object *newdoor(t_data *v, double ryd, int flag)
{
    t_object *nd;
    (void)v;
    
    nd = malloc(sizeof(t_object));
    if (!nd)
        ft_error("Allocate new door failed");
	nd->type = DOOR;
	(flag == WALL) && (nd->type = WALL);
	nd->rydis = ryd;
    nd->rydis_fbw = ryd * cos(Rad(v->orientation - v->ryc.ang));
	(flag == HORI || flag == VER) && (nd->hitw = flag);
	nd->x = v->x + ryd * cos(Rad(v->ryc.ang));
	nd->y = v->y + ryd * sin(Rad(v->ryc.ang));
	nd->next = NULL;
    return(nd);
}
t_object *newsprite(t_data *v, t_sprites sprite)
{
    t_object *nd;
    (void)v;
    
    nd = malloc(sizeof(t_object));
    if (!nd)
        ft_error("Allocate new door failed");
	nd->type = SPRITE;
	// nd->rydis = ryd;
    // nd->rydis_fbw = ryd * cos(Rad(v->orientation - v->ryc.ang));
	nd->rendered = 0;
	nd->i = sprite.i;
	nd->j = sprite.j;
	nd->x = sprite.xs;
	nd->y = sprite.ys;
	nd->angle = Deg(atan2(sprite.ys - v->y, sprite.xs - v->x)) - (v->orientation);
	nd->rydis = des_betw_2pt(sprite.xs, sprite.ys, v->x, v->y);
	nd->rydis_fbw = des_betw_2pt(sprite.xs, sprite.ys, v->x, v->y) * cos(Rad(nd->angle));
	// printf("x = %f y = %f ryd = %f\n", sprite.xs, sprite.ys, nd->rydis_fbw);
	nd->next = NULL;
    return(nd);
}

void	addobjt(t_object **lst, t_object *new)
{
	if (!lst)
		return ;
	if (*lst == NULL)
	{
		*lst = new;
		return ;
	}
	new->next = *lst;
	*lst = new;
}

void	clear_objt(t_object **lst)
{
	t_object	*c;

	if (!lst)
		return ;
	c = *lst;
	while (c)
	{
		if (c->next == NULL)
		{
			free((void*)*lst);
			break ;
		}
		c = c->next;
		free((void*)*lst);
		*lst = c;
	}
	*lst = NULL;
}